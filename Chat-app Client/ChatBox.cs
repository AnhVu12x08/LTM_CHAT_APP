...
namespace Chat_app_Client
{
    public partial class ChatBox : Form
    {
        ...
        private string currentRecipient = ""; // Add a variable to track the current recipient
        ...
        private void btnSend_Click(object sender, EventArgs e)
        {
            ...
            currentRecipient = txtReceiver.Text;
        ...
        private void tblGroup_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                string selectedGroup = tblGroup.Rows[e.RowIndex].Cells[0].Value.ToString();
                txtReceiver.Text = selectedGroup;
                currentRecipient = selectedGroup; // Update currentRecipient
                RequestChatHistory(selectedGroup);
            }
            catch (Exception)
            {
        ...
        private void tblUser_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            currentRecipient = selectedUser; // Update currentRecipient
            RequestChatHistory(selectedUser);
        }

        private async void RequestChatHistory(string otherUser)
        {
            // Send a request to the server for chat history
            Json request = new Json("REQUEST_HISTORY", otherUser);
            sendJson(request);

            // Wait for the server response (using a timeout for robustness)
            await Task.Delay(5000); // Wait for up to 5 seconds
        }

        private void DisplayChatHistoryOnUIThread(List<Messages> chatHistory)
        {
                    ...
                    switch (infoJson.type)
                    {
                        ...
                        case "MESSAGE":
                            if (infoJson.content != null)
                            {
                                ...
                                if (message != null)
                                {
                                    // Check if message is for the current recipient  (Simplified Condition)
                                    if (message.receiver == txtReceiver.Text)
                                    {
                                        AppendRichTextBox(message.sender, message.receiver, message.message, "");
                                    }
                                }
                        ...
                        case "CHAT_HISTORY":
                            if (infoJson.content != null)
                            {
                                ...
                                // Instead of directly displaying, signal the UI thread
                                this.BeginInvoke(new MethodInvoker(() => DisplayChatHistoryOnUIThread(chatHistory)));
                            }
                            break;
